...
void pong_mp_master_run(void) {
    ...
    while (1) {
        ...
        if (exit_now && !prev_exit) game_over = 1;
        prev_exit = exit_now;
        ...
        /* Aceptar solo valores útiles del esclavo */
        if (slave_y_rx != 0) {
            slave_y = iclamp((int)slave_y_rx * 2, 0, SCREEN_H - PAD_H);
        }   
        ...
        /* ── Fisica de la bola ── */
        //prev_my = master_y; prev_sy = slave_y;
        ...
        if (r != 0) {
            ...
            if (p1 >= WIN_SCORE || p2 >= WIN_SCORE) {
                ...
                while (1) {
                    ...
                    /* Actualizar paquete con game_over y ganador */
                    tx[4] = 0x04u | (p1_wins ? 0x08u : 0u);
                    Xil_Out32(SPI_DATA, 0u);
                    spi_master_transfer(tx[0]);
                    spi_master_transfer(tx[1]);
                    spi_master_transfer(tx[2]);
                    spi_master_transfer(tx[3]);
                    spi_master_transfer(tx[4]);
...
void pong_mp_slave_run(void) {
    ...
    while (1) {
        ...
        int flags       = rx[4];
        int game_over   = (flags >> 2) & 1;
        int p1_wins     = (flags >> 3) & 1;

        /* ── Detectar gol (cambio de score) para redibujar campo ── */
        int goal = (new_p1 != p1 || new_p2 != p2);
        p1 = new_p1; p2 = new_p2;

        if (!field_drawn || goal) {
            mp_draw_field(new_master_y, slave_y, new_bx, new_by, p1, p2);
            field_drawn = 1;
